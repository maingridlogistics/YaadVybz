 ...
 *
 * Pro:   Overview + Events + Businesses analytics.
 * Elite: Same + date range filter + Analytics Export (CSV via expo-sharing).
 * Free:  Locked / upgrade CTA.
 *
 * Security: All data comes from SECURITY DEFINER RPCs that enforce auth.uid()
 * ownership. No cross-user data exposure. No mock data.
 */
...
// ─── Event analytics row ──────────────────────────────────────────────────────
function EventRow({ ev }: { ev: EventAnalyticsRow }) {
  ...
  const hasRevenue = ev.revenue_minor > 0;
  return (
    <View style={er.wrap}>
      ...
      <View style={er.metrics}>
        <View style={er.metric}>
          ...
          <Text style={er.metricVal}>{ev.view_count}</Text>
        </View>
        <View style={er.metric}>
          <MaterialIcons name="check-circle-outline" size={11} color={Colors.textMuted} />
          <Text style={er.metricVal}>{ev.going_count} going</Text>
        </View>
        <View style={er.metric}>
          <MaterialIcons name="star-border" size={11} color={Colors.textMuted} />
          <Text style={er.metricVal}>{ev.interested_count} interested</Text>
        </View>
        {ev.tickets_sold > 0 && (
          <View style={er.metric}>
            <MaterialIcons name="confirmation-number" size={11} color={Colors.textMuted} />
            <Text style={er.metricVal}>{ev.tickets_sold} tickets</Text>
          </View>
        )}
        {hasRevenue && (
          <View style={er.metric}>
            <MaterialIcons name="payments" size={11} color={Colors.greenLight} />
            <Text style={[er.metricVal, { color: Colors.greenLight }]}>
              {formatRevenue(ev.revenue_minor, ev.revenue_currency)}
            </Text>
          </View>
        )}
        {ev.boost_impressions > 0 && (
          <View style={er.metric}>
            <MaterialIcons name="trending-up" size={11} color={Colors.gold} />
            <Text style={[er.metricVal, { color: Colors.gold }]}>{ev.boost_impressions} impr.</Text>
          </View>
        )}
      </View>
    </View>
...
const er = StyleSheet.create({
  ...
  metrics: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.sm, marginTop: 2 },
...
function BusinessRow({ biz }: { biz: BusinessAnalyticsRow }) {
  const statusColor: Record<string, string> = {
    live: Colors.greenLight, pending: '#FFD54F', rejected: '#FF5252',
  };
  const col = statusColor[biz.status] ?? Colors.textMuted;
  const ctr = safeCtr(biz.boost_clicks, biz.boost_impressions);
  return (
    <View style={er.wrap}>
      <View style={er.topRow}>
        <View style={[er.dot, { backgroundColor: col }]} />
      ...
      <View style={er.metrics}>
        ...
        <View style={er.metric}>
          ...
          <Text style={er.metricVal}>{biz.favorite_count} favs</Text>
        </View>
        {biz.review_count > 0 && (
          <View style={er.metric}>
            <MaterialIcons name="star" size={11} color="#FFD54F" />
            <Text style={er.metricVal}>{biz.avg_rating != null ? biz.avg_rating.toFixed(1) : '—'} ({biz.review_count})</Text>
          </View>
        )}
        {biz.boost_impressions > 0 && (
          <View style={er.metric}>
            <MaterialIcons name="trending-up" size={11} color={Colors.gold} />
            <Text style={[er.metricVal, { color: Colors.gold }]}>{biz.boost_impressions} impr.</Text>
          </View>
        )}
        {biz.boost_clicks > 0 && (
          <View style={er.metric}>
            <MaterialIcons name="touch-app" size={11} color={Colors.gold} />
            <Text style={[er.metricVal, { color: Colors.gold }]}>{biz.boost_clicks} clicks · {ctr} CTR</Text>
          </View>
        )}
      </View>
    </View>
...
export default function CreatorAnalyticsScreen() {
  ...
  const loadAll = useCallback(async (days: number | null) => {
    ...
    const [ovResult, evResult, bizResult] = await Promise.all([
      fetchAnalyticsOverview(days ?? undefined),
      fetchEventAnalytics(days ?? undefined),
      fetchBusinessAnalytics(days ?? undefined),
    ]);
  ...
  const handleExport = useCallback(async () => {
    if (!isElite) {
      Alert.alert('Elite Only', 'Analytics exports require an Elite subscription. Upgrade to access this feature.');
      return;
    }
    setExporting(true);
    try {
      ...
      if (!result.ok) {
        if ((result as any).code === 'UPGRADE_REQUIRED') {
          Alert.alert('Elite Required', result.error);
        } else {
  ...
  // ── Top boosted events for mini chart ─────────────────────────────────────
  const topEventsByViews = useMemo(
    () => [...events].sort((a, b) => b.view_count - a.view_count).slice(0, 5),
    [events]
  );
  const topBizByViews = useMemo(
    () => [...businesses].sort((a, b) => b.view_count - a.view_count).slice(0, 5),
    [businesses]
  );
    ...
    return (
      <View style={s.container}>
        <SafeAreaView edges={['top']} style={{ backgroundColor: Colors.background }}>
          <View style={s.header}>
            <Pressable onPress={() => navigation.canGoBack() ? navigation.goBack() : router.replace('/(tabs)/profile' as any)} style={s.backBtn}>
        ...
        <ScrollView contentContainerStyle={s.upgradeContent}>
          ...
          {[
            { icon: 'event', label: 'Event views, RSVPs, and ticket sales' },
            { icon: 'storefront', label: 'Business views, favorites, and reviews' },
            { icon: 'rocket-launch', label: 'Boost impressions and click-through rates' },
            { icon: 'download', label: 'Elite: export analytics to CSV' },
          ].map(({ icon, label }) => (
    ...
    return (
      <View style={s.container}>
        ...
        <View style={s.center}>
          ...
          <Pressable onPress={() => { setLoading(true); loadAll(dateRange.days).finally(() => setLoading(false)); }} style={s.retryBtn}>
  ...
  return (
    <View style={s.container}>
          ...
          <View style={s.dateRangeBar}>
            {DATE_RANGES.map((dr) => (
              ...
              >
                <Text style={[
                  s.dateRangeChipText,
                  dateRange.label === dr.label && s.dateRangeChipTextActive,
                ]}>
                  {dr.label}
                </Text>
              </Pressable>
      ...
      >
          ...
          <>
            ...
            <View style={s.section}>
              ...
              <View style={s.statsGrid}>
                <StatCard icon="event" iconColor="#42A5F5" label="Total Events" value={overview.total_events} />
                <StatCard icon="visibility" iconColor="#66BB6A" label="Event Views" value={overview.total_event_views} />
                <StatCard icon="check-circle" iconColor={Colors.greenLight} label="Going" value={overview.total_going} />
                <StatCard icon="star-border" iconColor="#FFD54F" label="Interested" value={overview.total_interested} />
                {overview.total_tickets_sold > 0 && (
                  <StatCard
                ...
                {overview.ticket_revenue_minor > 0 && (
                  <StatCard
                    icon="payments"
                    iconColor={Colors.greenLight}
                    label="Ticket Revenue"
                    value={formatRevenue(overview.ticket_revenue_minor, overview.ticket_currency)}
                    sub={overview.ticket_currency ?? ''}
                  />
                )}
                {overview.boost_event_impressions > 0 && (
                  <StatCard
                    icon="rocket-launch"
                    iconColor={Colors.gold}
                    label="Event Boost Impr."
                    value={overview.boost_event_impressions}
                  />
                )}
              </View>
            </View>
            ...
            <View style={s.section}>
              ...
              <View style={s.statsGrid}>
                <StatCard icon="storefront" iconColor="#FF6B35" label="Total Businesses" value={overview.total_businesses} />
                <StatCard icon="visibility" iconColor="#9C27B0" label="Business Views" value={overview.total_biz_views} />
                <StatCard icon="favorite-border" iconColor="#E91E63" label="Favorites" value={overview.total_biz_favorites} />
                <StatCard icon="rate-review" iconColor="#FFD54F" label="Reviews" value={overview.total_biz_reviews} />
                {overview.avg_biz_rating != null && (
                  <StatCard icon="star" iconColor="#FFD54F" label="Avg Rating" value={`${overview.avg_biz_rating.toFixed(1)} ★`} />
                )}
                {overview.boost_biz_impressions > 0 && (
                  <StatCard icon="trending-up" iconColor={Colors.gold} label="Biz Boost Impr." value={overview.boost_biz_impressions} />
                )}
                {overview.boost_biz_clicks > 0 && (
                  <StatCard
                    icon="touch-app"
                    iconColor={Colors.gold}
                    label="Biz Boost Clicks"
                    value={overview.boost_biz_clicks}
                    sub={`CTR: ${safeCtr(overview.boost_biz_clicks, overview.boost_biz_impressions)}`}
                  />
                )}
              </View>
            </View>

            {/* Top Events chart (if any) */}
            {topEventsByViews.length > 0 && (
              <View style={s.section}>
                <SectionTitle title="Top Events by Views" />
            ...
            {/* Top Businesses chart (if any) */}
            {topBizByViews.length > 0 && (
              <View style={s.section}>
                <SectionTitle title="Top Businesses by Views" />
                <View style={s.chartCard}>
                  <MiniBar
            ...
            {overview.total_events === 0 && overview.total_businesses === 0 && (
              <EmptyState
                ...
                sub="Your Event and Business performance will appear here after people start interacting with your content."
              />
            )}
        ...
        {activeTab === 'events' && (
          <>
            {events.length === 0 ? (
              <EmptyState
                icon="event"
                title="No event analytics yet"
                sub="Your Event performance will appear here after people start interacting with your Events."
              />
            ) : (
              <View style={s.section}>
                <SectionTitle title={`${events.length} Event${events.length !== 1 ? 's' : ''}`} />
        ...
        {activeTab === 'businesses' && (
          <>
            {businesses.length === 0 ? (
              <EmptyState
                icon="storefront"
                title="No business analytics yet"
                sub="Your Business performance will appear here after people start viewing and saving your Businesses."
              />
            ) : (
              <View style={s.section}>
                <SectionTitle title={`${businesses.length} Business${businesses.length !== 1 ? 'es' : ''}`} />
                <View style={{ gap: Spacing.sm }}>
        ...
        {/* Inline loading indicator (refresh) */}
        {loading && overview && (
