...
// ─── Scenario verification (run in development console if needed) ─────────────
//
// The function below is a dev-only test harness. It is never imported by
// production UI components — import only the comparators above.
//
// Usage: import { verifyRankingScenarios } from 'constants/rankingUtils';
//        verifyRankingScenarios(); // logs pass/fail table to console
export function verifyRankingScenarios(): void {
  const past = '2020-01-01';
  const future = '2099-12-31';
  const soon = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

  const base = (overrides: Partial<Event>): Event => ({
    id: 'x', title: '', description: '', type: '', typeLabel: '', eventTypes: [],
    parish: '', date: soon, startTime: '', endTime: '', venue: '', address: '',
    coverImage: '', flyerImages: [], ticketPrice: '', ticketLink: '', ageLimit: '',
    lineup: [], recurring: false, promoterId: '', promoterName: '',
    goingCount: 0, interestedCount: 0, featured: false, tags: [],
    status: 'live', boosted: false, promoterTier: 'free',
    ...overrides,
  });
  ...
  const scenarios: Array<{
    ...
    expected: 'A wins' | 'B wins' | 'tie';
  }> = [
    {
      label: 'A. Boosted Free vs Unboosted Elite — boost beats tier',
      a: base({ promoterTier: 'free', boosted: true, boostType: 'three_day', boostStatus: 'active', boostExpiresAt: expiry7d }),
      b: base({ promoterTier: 'elite', boosted: false }),
      comparator: compareBrowse,
      expected: 'A wins',
    },
    {
      label: 'B. Boosted Pro (7d) vs Boosted Elite (3d) — higher boost type wins',
      a: base({ promoterTier: 'pro', boosted: true, boostType: 'seven_day', boostStatus: 'active', boostExpiresAt: expiry7d }),
      b: base({ promoterTier: 'elite', boosted: true, boostType: 'three_day', boostStatus: 'active', boostExpiresAt: expiry7d }),
      comparator: compareBrowse,
      expected: 'A wins',
    },
    {
      label: 'B2. Same boost type: Boosted Pro vs Boosted Elite — tier breaks tie',
      a: base({ promoterTier: 'pro',   boosted: true, boostType: 'seven_day', boostStatus: 'active', boostExpiresAt: expiry7d }),
      b: base({ promoterTier: 'elite', boosted: true, boostType: 'seven_day', boostStatus: 'active', boostExpiresAt: expiry7d }),
      comparator: compareBrowse,
      expected: 'B wins',
    },
    {
      label: 'C. High-engagement Free vs low-engagement Elite (trending) — engagement wins',
      a: base({ promoterTier: 'free',  goingCount: 150, interestedCount: 50 }),
      b: base({ promoterTier: 'elite', goingCount: 2,   interestedCount: 1 }),
      comparator: compareTrending,
      expected: 'A wins',
    },
    {
      label: 'D. Expired-boosted Elite vs active organic Free — expired boost = 0',
      a: base({ promoterTier: 'elite', boosted: true, boostType: 'three_day', boostStatus: 'active', boostExpiresAt: '2020-01-01T00:00:00Z' }),
      b: base({ promoterTier: 'free',  boosted: false, goingCount: 5 }),
      comparator: compareBrowse,
      expected: 'B wins',
    },
    {
      label: 'E. Equal boost + equal engagement: Elite beats Pro as tiebreaker',
      a: base({ promoterTier: 'pro',   boosted: true, boostType: 'seven_day', boostStatus: 'active', boostExpiresAt: expiry7d, goingCount: 10 }),
      b: base({ promoterTier: 'elite', boosted: true, boostType: 'seven_day', boostStatus: 'active', boostExpiresAt: expiry7d, goingCount: 10 }),
      comparator: compareBrowse,
      expected: 'B wins',
    },
  ];
  ...
  scenarios.forEach(({ label, a, b, comparator, expected }) => {
    const result = comparator(a, b);
    const actual = result < 0 ? 'A wins' : result > 0 ? 'B wins' : 'tie';
